...
export default function HowItWorks() {
  const steps = [
    {
      ...
      icon: (
        <svg className="w-6 h-6 sm:w-7 sm:h-7 text-[#008744]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <section className="relative bg-gradient-to-r from-[#F0FDF4] via-white to-[#E8F5EE] py-12 sm:py-16 px-4 sm:px-6 lg:px-8 overflow-hidden">
      {/* Subtle background abstract curves */}
      <div className="absolute right-0 top-0 w-96 h-full bg-[radial-gradient(#008744_1px,transparent_1px)] [background-size:24px_24px] opacity-15 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10 sm:mb-12">
          ...
          {/* Main Heading */}
          <h2 className="text-3xl sm:text-4xl lg:text-[40px] font-bold text-[#04140D] tracking-tight font-['Outfit',sans-serif]">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6 xl:gap-8">
          ...
          <div className="w-full lg:flex-1 flex flex-col md:flex-row items-center justify-between gap-3 sm:gap-4">
            {steps.map((step, index) => (
              <div key={step.number} className="flex flex-col md:flex-row items-center w-full">
                {/* Step Card */}
                <div className="w-full bg-white rounded-2xl p-5 sm:p-6 shadow-[0_4px_25px_rgba(0,0,0,0.05)] border border-emerald-100/90 hover:shadow-xl hover:border-emerald-300 transition-all duration-300 flex items-start gap-4">
                  {/* Icon */}
                  <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-emerald-50 border border-emerald-100/80 flex items-center justify-center shrink-0">
                    {step.icon}
                  </div>

                  {/* Text Content */}
                  <div className="text-left">
                    <span className="text-[#008744] font-bold text-lg sm:text-xl font-['Outfit',sans-serif] block leading-none mb-1">
                      {step.number}
                    </span>
                ...
                {index < steps.length - 1 && (
                  <div className="py-2 md:py-0 md:px-2.5 text-slate-700 shrink-0">
                    <ArrowRight className="w-5 h-5 hidden md:block text-emerald-600" />
                    <span className="text-xs font-bold text-emerald-500 md:hidden">↓</span>
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="w-full lg:w-auto relative flex items-center justify-center shrink-0 pt-4 lg:pt-0">
            ...
            <div className="absolute -top-6 -right-2 sm:right-0 lg:-top-8 lg:-right-4 z-10 pointer-events-none select-none">
              <span className="text-xl sm:text-2xl text-[#008744] font-['Caveat',cursive] leading-tight font-bold -rotate-6 block text-left">
                From Admission <br />
                to Alumni <br />
                — All in One Place
              </span>
              <svg
                className="w-10 h-10 text-[#008744] ml-4 -mt-1 opacity-90"
                viewBox="0 0 50 50"
                fill="none"
                stroke="currentColor"
                strokeWidth="2.5"
            ...
            {/* Seamless 3D Graduation Cap & Diploma Image */}
